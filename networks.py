...
def upscale2doriginal(x, factor=2):
...
def upscale2d(x, factor=2):
    ...
    with tf.variable_scope('Upscale2D'):
        ...
        xcomp = tf.dtypes.cast(x,tf.complex64)
        xf = tf.dtypes.cast(tf.fft2d(xcomp),tf.float32)


        #xds_f =tf.dtypes.cast(tf.nn.avg_pool(tf.dtypes.cast(tf.fft2d(xcomp),tf.float32), ksize=ksize, strides=ksize, padding='VALID', data_format='NCHW'),tf.complex64)
        #xds = tf.ifft(xds_f)

        s = x.shape
        xf = tf.reshape(xf, [-1, s[1], s[2], 1, s[3], 1])
        xf = tf.tile(xf, [1, 1, 1, factor, 1, factor])
        xf = tf.reshape(xf, [-1, s[1], s[2] * factor, s[3] * factor])

        x = tf.dtypes.cast(tf.ifft(tf.dtypes.cast(xf, tf.complex64)),tf.float32)
        return x
...
def downscale2doriginal(x, factor=2):
    assert isinstance(factor, int) and factor >= 1
    if factor == 1: return x
...
def downscale2d(x, factor=2):
    ...
    with tf.variable_scope('Downscale2D'):
        ...
        #return tf.nn.avg_pool(x, ksize=ksize, strides=ksize, padding='VALID', data_format='NCHW') # NOTE: requires tf_config['graph_options.place_pruned_graph'] = True
        return tf.dtypes.cast(xds,tf.float32)
