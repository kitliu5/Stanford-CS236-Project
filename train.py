...
class TrainingSchedule:
    def __init__(
        ...
        ##tick_kimg_dict          = {4: 160, 8:20, 16:20, 32:20, 64:20, 128:20, 256:20, 512:20, 1024:10}): # Resolution-specific overrides.
        ##tick_kimg_dict          = {4: 40, 8:20, 16:20, 32:20, 64:20, 128:40, 256:40, 512:20, 1024:10}): # Resolution-specific overrides.
...
def train_progressive_gan(
    ...
    ##true_dataset_dir = 'E:/progressive_growing_of_gans-master/n16/true_dataset'
    maintenance_start_time = time.time()
    training_set = dataset.load_dataset(data_dir=config.data_dir, verbose=True, **config.dataset)

    #R add true_set
    ##true_set = dataset.load_dataset(data_dir=true_dataset_dir, verbose=True, **config.dataset)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                ...
                #G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, training_set=training_set, true_set=true_set, minibatch_size=minibatch_split, **config.G_loss)
